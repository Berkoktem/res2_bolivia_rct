####################################################################################
####### Object:  Create a pixel-level dataset for the model          
...
for(community in dbf$ID_OTB){
  ...
  com_shp_name <- paste0(shp_com_dir,community,".shp")
  writeOGR(com_shp,com_shp_name,community,"ESRI Shapefile",overwrite_layer = T)
  
  com_map_name <- paste0(map_com_dir,community,".tif")
  ...
  system(sprintf("gdal_translate -co COMPRESS=LZW  -projwin %s %s %s %s %s %s",
                 ...
                 e@ymin,
                 the_map,
                 paste0(map_com_dir,"tmp_",community,".tif")
                 ))
  ...
  file.remove(paste0(map_com_dir,"tmp_",community,".tif"))
  ...
  plenty <- min(500*nb_pts,ncell(map))
...
write.csv(points,paste0(mod_dir,"points_loss_forest_1000_20171205_th",thresh_gfc,".csv"),row.names=F)
...
writeOGR(sp_df,
         paste0(mod_dir,"sampling_20171205_th",thresh_gfc,".shp"),
         layer=paste0("sampling_20171205_th",thresh_gfc),
...
summary(df1)
length(unique(df1$ID_OTB_II))
write.csv(df1,
          paste0(mod_dir,"sampling_pixel_dataset_20171205_th",thresh_gfc,".csv"),
          row.names = F)
head(df1)

min(table(df1$ID_OTB_II))
max(table(df1$ID_OTB_II))
table(df1$ID_OTB_II)
