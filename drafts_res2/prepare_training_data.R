...
################################################################################
system(sprintf("oft-clip.pl %s %s tmp.tif",im_input,slp_input))
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW tmp.tif %s",slp_clip))
...
################################################################################
system(sprintf("(echo 1; echo \" %s %s / 1 < 0 %s %s / 1 < 0 1 ? ?\") | oft-calc %s %s",NIR,GRN,NIR,RED,im_input,paste0(outdir,"/","tmp_dark.tif")))
#system(sprintf("(echo 1; echo \" %s %s >\") | oft-calc -ot Byte %s %s",NIR,SIR,im_input,paste0(outdir,"/","tmp_ndwi.tif")))
...
system(sprintf("gdal_translate -ot byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_shadow.tif"),shd_msk))
...
system(sprintf("gdal_translate -ot byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_water.tif"),wat_msk))
system(sprintf(paste0("rm ",outdir,"/","tmp*.tif")))
...
################################################################################
system(sprintf("(echo 1; echo \"#1 #2 + #3 + #4 + 0 > 0 1 ? \") | oft-calc -ot Byte %s %s",im_input,paste0(outdir,"/","tmp_mask.tif")))
system(sprintf("gdal_translate -ot byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_mask.tif"),data_mask))
system(sprintf(paste0("rm ",outdir,"/","tmp*.tif")))
...
system(sprintf("gdal_translate -ot byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_clear_land.tif"),land_msk))
system(sprintf(paste0("rm ",outdir,"/","tmp*.tif")))
...
system(sprintf("(echo %s; echo 50) | oft-kmeans -o %s -i %s -um %s",sg_km,paste0(outdir,"/","tmp_km_se.tif"),im_input,land_msk))

## Sieve results with a 8 connected component rule
system(sprintf("gdal_sieve.py -8 %s %s",paste0(outdir,"/","tmp_km_se.tif"),paste0(outdir,"/","tmp_sieve_km_se.tif")))
system(sprintf("gdal_translate -ot byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_sieve_km_se.tif"),all_sg_km))
...
################################################################################
system(sprintf("oft-clump -i %s -o %s -um %s",all_sg_km,paste0(outdir,"/","tmp_all_seg_id.tif"),all_sg_km))
system(sprintf("gdal_translate -ot Float32 -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_all_seg_id.tif"),all_sg_id))
...
################################################################################
## Clip the PBS product to the extent of the interest tile
system(sprintf("oft-clip.pl %s %s tmp.tif",im_input,pbs_input))
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW tmp.tif %s",pbs_clip))
system(sprintf("rm tmp.tif"))

################################################################################
## Inject PBS  values inside the selected segments
system(sprintf("bash oft-segmode.bash %s  %s  %s ",sel_sg_id,pbs_clip,paste0(outdir,"/","tmp_sl_sg_pbs.tif")))
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_sl_sg_pbs.tif"),sel_sg_pbs))
...
system(sprintf("gdal_calc.py -A %s -B %s -C %s --outfile=%s --calc=\"%s\"",
               ...
               noch_msk,
               slp_clip,
               paste0(outdir,"/","tmp_sl_sg_pbs_nochange.tif"),
               paste0("A*B*(C<",th_wat,")")
               ))

system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_sl_sg_pbs_nochange.tif"),sel_sg_pbs))
system(sprintf(paste0("rm ",outdir,"/","tmp_*.tif")))

 
################################################################################
## Clump again the selected segments to get unique IDs including water & shadows
system(sprintf("oft-clump -i %s -o %s -um %s",sel_sg_pbs,paste0(outdir,"/","tmp_sel_seg_id.tif"),sel_sg_pbs))
system(sprintf("gdal_translate -ot UInt16 -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_sel_seg_id.tif"),sel_sg_id))
system(sprintf(paste0("rm ",outdir,"/","tmp_*.*")))
