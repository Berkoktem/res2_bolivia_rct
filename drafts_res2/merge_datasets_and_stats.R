...
legend_pbs <- data.frame(cbind(
  c("clouds","snow_temp","snow",
    ...
    "empty","empty","empty",
    "forest_deciduous_closed_humid","cultivated_deciduous_open_humid",
    "empty","empty","empty",
...
## Create a new reclass column: 1==forest loss, 2==forest stable, 3==the rest
df1$recl <- 3
...
#     rowSums(df1[,grep(pattern = "forest",names(df1))]) > (0.99*df1$sg_sz) & # The RapidEye classification says "Forest" for more than 70% of the segment
#     df1$imad > 100, #IMAD indicates some change is occuring
#   ]$recl <- 1
# }, error=function(e){cat("Configuration impossible \n")}
# )
...
system(sprintf("(echo %s; echo 1; echo 1; echo 2; echo 0) | oft-reclass -oi  %s %s",reclass,paste0(mergedir,"/","tmp_reclass.tif"),segs_id))
system(sprintf("gdal_translate -ot byte -co COMPRESS=LZW %s %s",paste0(mergedir,"/","tmp_reclass.tif"),chg_class))
