...
dbf <- read.dbf(paste0(anmi_dir,"/","community_boundaries/","communities_UTM20_20170913.dbf"))
...
his <- read.table(
  paste0(zonal_dir,"zonal_results_edited_20170919.txt")
)
...
#######          COMPUTE ZONAL STATS
####################################################################################
############################### By ANMI community ARA coverage
his_ara        <- read.table(paste0(zonal_dir,"zonal_ara_coverage_20170920.txt"))
names(his_ara) <- c("zone","total_px_ara","px_no_ara",paste0("ara_level_",1:3))
pixel_size     <- res(raster(paste0(workdir,"results_edited_20170613.tif")))[1]

his_ara$total_ha_ara <- his_ara$total_px_ara*pixel_size*pixel_size/10000
his_ara$no_ara_ha    <- his_ara$px_no_ara*pixel_size*pixel_size/10000
his_ara[,paste0("ara_level_",1:3,"_ha")] <- his_ara[,paste0("ara_level_",1:3)]*pixel_size*pixel_size/10000
...
his_tc <- read.table(paste0(zonal_dir,"zonal_gfc_tc_",thresh_gfc,".txt"))
...
df <- merge(x = df1,
            ...
            y = dbf[,c("NOMPRED","cont_treat","ID_OTB","int_code","area_ha")],
            by.x="zone",
            by.y="int_code",
            all=T)
...
out <- df[,c( "zone","NOMPRED","cont_treat","ID_OTB","area_ha",
              "total_ha_res2","nodata_ha_res2","loss_ha_res2","forest_ha_res2","non_forest_ha_res2","water_ha_res2",
              "openf_loss_ha_res2","openf_stable_ha_res2",
              "shrub_loss_ha_res2","shrub_stable_ha_res2",
              "total_ha_tc","no_tc_ha","tc_ha",
              ...
              "total_ha_ara","no_ara_ha",paste0("ara_level_",1:3,"_ha")
              )
          ]

write.csv(out,paste0(rootdir,"stats_db_ecoregions_20170920.csv"),row.names = F)
