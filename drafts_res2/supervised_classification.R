...
system(sprintf("oft-stat -i %s -o %s -um %s -nostd",im_input,img_sg_st,all_sg_id))

## Extract spectral signature of the image on the training polygons ids --> the training data spectral info
system(sprintf("oft-stat -i %s -o %s -um %s -nostd",im_input,img_tr_st,sel_sg_id))

## Extract values of the training data on the training polygons ids --> the training data class info
system(sprintf("oft-stat -i %s -o %s -um %s -nostd",sel_sg_pbs,tra_tr_st,sel_sg_id))
...
names(trainer_class) <- c("tr_id","tr_sz","pbs_code")

table(trainer_class$pbs_code)

################### Get training data
training <- merge(img_trng_spec,trainer_class)

training <- training[training$pbs_code != 0 ,c("pbs_code",paste0("b",1:nband))]
...
nb_class <- length(levels(as.factor(training$pbs_code)))
print("training is read and ratio are calculated, reading data....")


### Run the classification model
  fit <- randomForest(as.factor(pbs_code) ~ . ,ntree=400, mtry=6, data=training)
  importance(fit)
...
system(sprintf("(echo %s; echo 1; echo 1; echo 2; echo 0) | oft-reclass -oi  %s %s",img_clas,paste0(outdir,"/","tmp_reclass.tif"),all_sg_id))
system(sprintf("(echo 1; echo \"#1 #2 * #3 * #4 *\") | oft-calc -ot Byte %s %s",im_input,paste0(outdir,"/","tmp_mask.tif")))
system(sprintf("gdal_calc.py -A %s -B %s --outfile=%s --calc=%s",paste0(outdir,"/","tmp_mask.tif"),paste0(outdir,"/","tmp_reclass.tif"),paste0(outdir,"/","tmp_reclass_masked.tif"),"\"(A>0)*B\""))
system(sprintf("gdal_translate -ot byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_reclass_masked.tif"),all_sg_rf))
...
system(sprintf("gdal_calc.py -A %s -B %s -C %s --outfile=%s --calc=\"%s\"",
               ...
               paste0("(C>0)*",pbs_shd_class," + (B>0)*",pbs_wat_class," + (B<1)*(C<1)*A")
))

system(sprintf("gdal_translate -ot byte -co COMPRESS=LZW %s %s",paste0(outdir,"/","tmp_rf_wat_shd_mask.tif"),output_rf))
system(sprintf(paste0("rm ",outdir,"/","tmp*.tif")))
