...
the_map <- paste0(gfc_dir,"defor_map_gfc16_utm.tif")
...
system(sprintf("oft-stat -i %s -o %s -um %s",
               map,
               paste0(mod_dir,"stats_map_change_gfc16.txt"),
               map
               ))

stats <- read.table(paste0(mod_dir,"stats_map_change_gfc16.txt"))[,1:2]
...
dir.create(paste0(mod_dir,"map_communities_gfc16/"))
dir.create(paste0(mod_dir,"shape_communities_gfc16/"))

################ Loop through communities to extract the number of points for each submap
for(community in dbf$ID_OTB[57:128]){
...
write.csv(points,paste0(mod_dir,"points_loss_forest_1000_20171129.csv"),row.names=F)
...
writeOGR(sp_df,
         paste0(mod_dir,"sampling_20171129.shp"),
         layer="sampling_20171129",
...
write.csv(df1,
          paste0(mod_dir,"sampling_pixel_dataset_20171129.csv"),
          row.names = F)
head(df1)
