...
pbs_clip  <- paste0(outdir,"/",basename,"_pbs.tif")       # clip of PBS over the tile
...
pbs_wat  <- paste0(outdir,"/",basename,"_pbs_wat.tif")    # clip of PBS over the tile with water mask merged

sel_sg_pbs<- paste0(outdir,"/",basename,"_selsgpbs.tif")  # majority PBS class for each selected segment
